...
#### ICS란 산업생산에서 사용되는 감시 제어 및 데이터 수집(SCADA, Supervisory Control And Data Acquision), 분산제어시스템(DCS, Distributed Control System), 프로그래머블 로직 컨트롤러(Programmable Logic Controller)와 Field Device 같은 시스템을 포함한 여러 종류의 제어시스템을 말한다. ICS는 하나의 산업 목표(제조, 운동 등)를 달성하기 위해 함께 작동하는 제어 구성요소(전기, 기계, 유압, 공압)의 조합들로 구성된다.
