...
###### 영상회의 환경에서 영상회의 개설자는 1. 회의실 입장을 위한 암호를 설정해야 한다. 2. 영상회의실 주소를 고정하여 사용하지 않아야 한다. 3. 초대자와 참석자의 일치 여부를 확인해야 한다. 4. 최신 보안 업데이트 상태로 단말기를 관리해야 한다. 영상회의 참가자는 1. 자동 로그인을 사용하지 않아야 한다. 2. 암호화 을 설정하여 진행해야 한다. 3. 회의 내용이 노출되지 않도록 업무 전용 공간을 확보하거나 이어폰을 사용한다.
